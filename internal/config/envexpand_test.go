...
func TestExpandAIConfigEnv(t *testing.T) {
	os.Setenv("TEST_AI_KEY", "ai-secret")
	os.Setenv("TEST_AI_BASE_URL", "https://llm.example/v1")
	os.Setenv("TEST_AI_MODEL", "lab-model")
	defer os.Unsetenv("TEST_AI_KEY")
	defer os.Unsetenv("TEST_AI_BASE_URL")
	defer os.Unsetenv("TEST_AI_MODEL")

	cfg := &AIConfig{
		DefaultChannel: "${TEST_AI_CHANNEL:-lab}",
	...
	ExpandAIConfigEnv(cfg)

	channel := cfg.Channels["lab"]
	if cfg.DefaultChannel != "lab" || channel.APIKey != "ai-secret" || channel.BaseURL != "https://llm.example/v1" || channel.Model != "lab-model" {
		t.Fatalf("AI config was not expanded: default=%q channel=%+v", cfg.DefaultChannel, channel)
	}
}
