// Package agents loads Markdown agent definitions from the agents/ directory (sub-agents plus an optional main agent defined by orchestrator.md or kind: orchestrator).
package agents
...
// OrchestratorMarkdownFilename is a fixed filename; when present, it defines the Deep main agent and is excluded from the sub-agent list.
const OrchestratorMarkdownFilename = "orchestrator.md"

// OrchestratorPlanExecuteMarkdownFilename is the dedicated Markdown filename for the planning-side main agent in plan_execute mode.
const OrchestratorPlanExecuteMarkdownFilename = "orchestrator-plan-execute.md"

// OrchestratorSupervisorMarkdownFilename is the dedicated Markdown filename for the main agent in supervisor mode.
const OrchestratorSupervisorMarkdownFilename = "orchestrator-supervisor.md"

// FrontMatter corresponds to the fields in a Markdown file header (matching the documentation examples).
type FrontMatter struct {
	...
	Kind          string      `yaml:"kind,omitempty"` // orchestrator = main agent (the orchestrator.md filename alone can also be used)
}

// OrchestratorMarkdown is a main-agent (Deep coordinator) definition parsed from the agents directory.
...
// MarkdownDirLoad is the result of one scan of the agents directory (the sub-agent list excludes main-agent files).
type MarkdownDirLoad struct {
	SubAgents               []config.MultiAgentSubConfig
	Orchestrator            *OrchestratorMarkdown // Deep main agent
	OrchestratorPlanExecute *OrchestratorMarkdown // plan_execute planning main agent
	OrchestratorSupervisor  *OrchestratorMarkdown // supervisor main agent
	FileEntries             []FileAgent           // Includes main agents and all sub-agents for the management API list
}

// OrchestratorMarkdownKind returns the main-agent type by fixed filename: deep, plan_execute, or supervisor; otherwise it returns an empty string.
...
// IsOrchestratorMarkdown reports whether a file occupies the **Deep** main-agent slot: orchestrator.md or kind: orchestrator (excluding dedicated plan_execute/supervisor filenames).
...
// IsOrchestratorLikeMarkdown reports whether a file should be displayed as a main-agent-type file in the frontend/API.
...
// WantsMarkdownOrchestrator reports, before saving, whether a file would be treated as a main agent (used for uniqueness validation).
...
// SlugID generates a usable agent ID from name (lowercase and hyphenated).
...
// sanitizeEinoAgentID normalizes the Deep main agent's Name in Eino to lowercase ASCII letters, numbers, and hyphens, matching the default cyberstrike-deep.
...
func subAgentFromFrontMatter(filename string, fm FrontMatter, body string) (config.MultiAgentSubConfig, error) {
	...
	if name == "" {
		return out, fmt.Errorf("agents: %s is missing the name field", filename)
	}
...
// LoadMarkdownAgentsDir scans the agents directory, separating at most one main agent for each of Deep, plan_execute, and supervisor, plus the remaining sub-agents.
	...
	for _, n := range names {
		...
		case "plan_execute":
			if out.OrchestratorPlanExecute != nil {
				return nil, fmt.Errorf("agents: only one %s may be defined; %s already exists", OrchestratorPlanExecuteMarkdownFilename, out.OrchestratorPlanExecute.Filename)
			}
		...
		case "supervisor":
			if out.OrchestratorSupervisor != nil {
				return nil, fmt.Errorf("agents: only one %s may be defined; %s already exists", OrchestratorSupervisorMarkdownFilename, out.OrchestratorSupervisor.Filename)
			}
			orch, err := orchestratorFromParsed(n, fm, body)
			if err != nil {
				return nil, fmt.Errorf("%s: %w", n, err)
		...
		if IsOrchestratorMarkdown(n, fm) {
			if out.Orchestrator != nil {
				return nil, fmt.Errorf("agents: only one main agent (Deep coordinator) may be defined; existing %s conflicts with %s", out.Orchestrator.Filename, n)
			}
			orch, err := orchestratorFromParsed(n, fm, body)
			if err != nil {
				return nil, fmt.Errorf("%s: %w", n, err)
...
// LoadMarkdownSubAgents reads all sub-agent .md files in a directory (excluding the main agent defined by orchestrator.md or kind: orchestrator).
...
// FileAgent represents one Markdown file and its parsed result.
...
// LoadMarkdownAgentFiles lists all .md files in a directory, including main agents, for use by the management API.
...
// MergeYAMLAndMarkdown merges sub_agents from config.yaml with Markdown definitions: Markdown overrides YAML for matching IDs, and Markdown-only entries are appended after the YAML order.
...
// BuildMarkdownFile serializes a configuration into Markdown that can be written back to disk.
