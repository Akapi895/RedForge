...
Restore the previous binary, previous `config.yaml`, and the pre-upgrade `data/` directory.
...
- Delete its external MCP configuration.
- Remove its tools from roles and allowlists.
- Check whether running Agent tasks still hold the old tool context.
...
- Restore the previous `data/knowledge.db`.
- Lower `batch_size` and rebuild the index.
