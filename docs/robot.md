...
4. **Enable the bot and choose Stream mode**
   ...
   - **Critical**: set message reception to **“Stream mode”**. If you only enable “HTTP callback” or do not select Stream, CyberStrikeAI will not receive messages.
   - Save.
...
|--------------------------------------|------------|-------|
| Get and send direct and group messages | `im:message` | Base permission for sending and receiving; **required**. |
| Receive @bot messages in group chat | `im:message.group_at_msg:readonly` | Required for group chat when users @ the bot. |
| Read direct messages from users to the bot | `im:message.p2p_msg:readonly` | **Required** for 1:1 chat; otherwise no response in private chat. |
| Get direct and group messages | `im:message:readonly` | **Required** to read message content. |
...
- **1:1 chat**: Open the bot’s private chat in Lark and send a message such as “help”; no @ needed.
...
6. **Test in WeChat**
   Open the **private chat** with the CyberStrikeAI bot in WeChat and send “help” or any text.
...
| Bound but no replies | Check logs for the WeChat iLink polling-started and message-received events; ensure “Enable WeChat robot” is on |
...
```text
Personal WeChat: send directly in a private chat; group chat is not supported.
DingTalk/Lark: direct messages need no @; group messages must @ the bot.
```
...
If the JSON response contains a reply with the CyberStrikeAI bot command header, command handling works. `help`, `version`, and `whoami` work before binding. `list`, `current`, and normal AI messages enforce RBAC: the test `platform + user_id` must already be bound or exactly match the service-account allowlist.
...
3. **Application logs**
   - On startup: the WeChat iLink polling-started event;
   - After sending a message: the WeChat message-received event; if missing, binding may have failed or `bot_token` is invalid—try **Re-bind**.
   - A WeChat iLink polling error followed by automatic reconnect: wait for auto-reconnect or restart.
...
3. **Application logs**
   - On startup you should see DingTalk Stream connecting and started/waiting-for-messages events.
   - If the DingTalk Stream long-lived connection exits with an error, it is usually an incorrect **Client ID / Client Secret** or **Stream not enabled** in the open platform.
   - After sending a message in DingTalk, you should see a DingTalk message-received event in the logs; if not, the platform is not pushing to this app (check that the bot is enabled and **Stream mode** is selected).
