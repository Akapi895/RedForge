...
Documentation files live directly under `docs/` and should keep stable filenames:
...
The check verifies local links, fenced code blocks, documentation index coverage, and the Go version documented by the root README. Keep versioned examples derived from authoritative files such as `go.mod` and `config.example.yaml` whenever possible.
