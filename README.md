<div align="center">
  <img src="images/logo.png" alt="RedForge Logo" width="200" >
</div>
...
This repository is an independent RedForge working copy for runtime validation
on a separate Linux machine. It is not connected to an upstream Git remote;
create and configure your personal remote when ready.

### Active English runtime

- The browser locale is forced to `en-US` and the language selector is removed.
- The active agent and role directories are `agents/` and `roles/`.
- Built-in agent, blackboard, shell, and HITL guidance is in English.
- The main JavaScript locale fallback is English.
- Legacy upstream fixtures or documents may still contain historical strings;
  they are not selected by the default runtime paths.

### Configure an LLM

RedForge does not require a dotenv-based `.env.example` flow. Copy
`config.example.yaml` to `config.yaml` and configure `ai.default_channel` plus
the matching `ai.channels.<id>` entry:
...
For another OpenAI-compatible endpoint, replace `base_url` and `model`. Export
the key before starting the server; RedForge expands the environment variable
without requiring a dotenv file:

```bash
export CYBERSTRIKE_AI_API_KEY='replace-with-a-secret'
```

Never commit a real key. The same environment expansion is supported in the
legacy `openai` compatibility block and external MCP configuration. After
startup, validate the channel from **System Settings -> Basic Settings -> AI
Channel Configuration -> Test connection**. Use the same model and budget for
all benchmark modes.

### Validate the gateway before enabling tools

A basic chat completion is not enough for the RedForge agent loop. Validate
standard function calling and, for the streaming UI, OpenAI-compatible SSE
with this harmless probe:

```bash
curl -sS -X POST \
  "https://stream-netmind.viettel.vn/aigw/ai/v1/chat/completions" \
  -H "Authorization: Bearer ${CYBERSTRIKE_AI_API_KEY}" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "MiniMax/MiniMax-M3",
    "messages": [{"role":"user","content":"Call safe_probe with value hello. Do not explain."}],
    "tools": [{"type":"function","function":{
      "name":"safe_probe",
      "description":"Harmless capability probe; performs no network or filesystem action.",
      "parameters":{"type":"object","properties":{"value":{"type":"string"}},"required":["value"]}
    }}],
    "tool_choice":{"type":"function","function":{"name":"safe_probe"}}
  }'
```

The response should contain `choices[0].message.tool_calls`. If the gateway
rejects `tools` or `tool_choice`, use it for chat and report drafting only; it
is not suitable for the RedForge agent loop yet. If `GET /models` is
unavailable, enter the model ID manually. For the first run, keep
`knowledge.enabled: false`, `vision.enabled: false`, and `reasoning.mode: off`.
The demonstrated endpoint proves chat completion only; RAG requires a
compatible `/embeddings` service and vision requires a compatible multimodal
model.

### Safety boundary

Use an isolated, explicitly authorized lab for runtime validation. Start with
harmless tools and read-only enumeration. Keep approval enabled for actions
that can change state, establish a session, write files, execute payloads, or
move between hosts.

### Create a personal remote

After creating an empty repository, run these commands from the RedForge root:

```bash
git init
git branch -M main
git add .
git commit -m "Create English-only RedForge validation fork"
git remote add origin <your-repository-url>
git push -u origin main
```

Review `config.yaml`, database files, generated reports, and test evidence
before committing.
...
├── roles/               # Role configurations (16 predefined security testing roles)
